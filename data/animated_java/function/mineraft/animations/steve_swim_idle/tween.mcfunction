...
$scoreboard players set @s aj.steve_swim_idle.frame $(to_frame)
scoreboard players operation #this aj.i = @s aj.tween_duration
tag @s add aj.transforms_only
execute at @s run function animated_java:mineraft/animations/steve_swim_idle/zzz/apply_frame {frame: 0}
$execute at @s run function animated_java:mineraft/animations/steve_swim_idle/zzz/apply_frame {frame: $(to_frame)}
