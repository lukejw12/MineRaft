...
tag @s[tag=planks_click,type=interaction] add held_planks
tag @s[tag=planks_click,type=interaction] remove planks_click
execute as @s[tag=has_interaction,tag=held_planks] at @s on target if items entity @s weapon.mainhand minecraft:potion[minecraft:potion_contents={potion:"minecraft:water"}] run tag @e[type=interaction,tag=has_interaction,limit=1,sort=nearest,distance=0..] add water_click
