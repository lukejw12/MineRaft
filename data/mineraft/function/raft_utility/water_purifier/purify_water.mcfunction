...
execute as @s[tag=has_interaction] at @s on target if items entity @s weapon.mainhand minecraft:potion[minecraft:potion_contents={potion:"minecraft:water"}] run tag @e[type=interaction,tag=has_interaction,limit=1,sort=nearest,distance=0..] add water_click
