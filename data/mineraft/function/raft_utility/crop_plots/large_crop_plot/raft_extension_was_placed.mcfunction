...
execute at @s if entity @s[tag=valid] if block ~ ~-1 ~ #valid_center_blocks run summon marker ~ ~ ~ {Tags:["tree_plot_active"]}
...
execute at @s if entity @s[tag=valid] if block ~ ~-1 ~ #valid_center_blocks run summon interaction ~ ~ ~ {width:2.2,height:4,response:true,Tags:["tree_plot_interact"]}
execute at @s if entity @s[tag=valid] if block ~ ~-1 ~ #valid_center_blocks run summon item_display ~ ~0.29 ~ {item_display:"ground",Tags:["tree_plot_facade"],brightness:{sky:15,block:0},transformation:{left_rotation:[0f,0f,0f,1f],right_rotation:[0f,0f,0f,1f],translation:[0f,0f,0f],scale:[1f,1f,1f]},item:{id:"minecraft:barrier",count:1,components:{"minecraft:item_model":"minecraft:tree_crop_plot"}}}
