execute positioned ~ ~ ~ anchored eyes run summon marker ~ ~ ~ {Tags:["mr.small_crop_plot_raycast","mr.new_ray"]}
...
scoreboard players set @e[type=marker,tag=mr.small_crop_plot_raycast,limit=1,sort=nearest] mr.raycast_dist 50

execute as @e[type=marker,tag=mr.small_crop_plot_raycast,limit=1,sort=nearest] at @s run function mineraft:structures/crop_plot/basic/small/raycast/loop