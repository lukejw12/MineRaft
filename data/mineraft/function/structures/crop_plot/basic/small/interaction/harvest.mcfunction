...
execute as @e[type=item_display,tag=mr.small_crop_display] if score @s mr.crop_plot_id = #crop_plot_id mr.data if data entity @s {data:{state:"stage3"}} run tag @s add mr.harvestable
...
execute as @e[tag=mr.target_crop_plot] if score @s mr.purifier_fuel matches ..0 run function mineraft:structures/crop_plot/basic/small/interaction/reset