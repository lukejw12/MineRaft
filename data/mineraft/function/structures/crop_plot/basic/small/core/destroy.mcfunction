...
execute as @e[type=item_display,tag=mr.small_crop_plot_display] if score @s mr.crop_plot_id = #crop_plot_id mr.data at @s run function mineraft:grid/block/release_pair_at_pos

execute as @e[type=item_display,tag=mr.small_crop_plot_display] if score @s mr.crop_plot_id = #crop_plot_id mr.data run kill @s
