...
execute align xyz positioned ~0.5 ~1 ~0.5 if entity @e[type=item_display,tag=mr.support_display,distance=..0.5] run return run function mineraft:structures/support/raycast/preview/hide
...
execute align xyz as @e[type=item_display,tag=mr.support_preview] if score @s mr.link = #player_link mr.data run tp @s ~0.5 ~1.21 ~0.5
