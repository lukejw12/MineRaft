...
execute positioned ~ ~ ~ anchored eyes run summon marker ~ ~ ~ {Tags:["mr.support_preview_raycast","mr.new_preview_ray"]}
...
execute as @e[type=marker,tag=mr.new_preview_ray,limit=1,sort=nearest] run data modify entity @s data.support_type set from storage mineraft:support support_type
...
scoreboard players set @e[type=marker,tag=mr.support_preview_raycast,limit=1,sort=nearest] mr.raycast_dist 50

execute as @e[type=marker,tag=mr.support_preview_raycast,limit=1,sort=nearest] at @s run function mineraft:structures/support/raycast/preview/loop