...
execute as @e[type=item_display,tag=mr.purifier_display] at @s if score @s mr.purifier_id = #purifier_id mr.data if entity @e[type=item_display,tag=mr.wall_display,distance=..3] run function mineraft:structures/water_purifier/restore_wall_trapdoor
