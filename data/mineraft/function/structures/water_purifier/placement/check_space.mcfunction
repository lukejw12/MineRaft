...
execute align xyz positioned ~ ~1 ~ run function mineraft:structures/water_purifier/spawn/spawn

function mineraft:core/structure/cache_item
execute as @a if score @s mr.link = #player_link mr.data unless entity @s[gamemode=creative] run clear @s *[custom_data~{mr.purifier:1b}] 1
