...
execute positioned ~0.5 ~ ~0.5 as @e[type=item_display,tag=mr.surface,distance=..10,limit=1,sort=nearest] if data entity @s data.grid.height_blocked run return fail

scoreboard players set #has_capacity mr.data 0
execute positioned ~0.5 ~ ~0.5 as @e[type=item_display,tag=mr.surface,distance=..10,limit=1,sort=nearest] if data entity @s {data:{grid:{has_support:1b}}} run function mineraft:grid/support/has_capacity
...
execute as @a if score @s mr.link = #player_link mr.data unless entity @s[gamemode=creative] run clear @s *[custom_data~{mr.platform:1b}] 1
...
execute as @e[type=item_display,tag=mr.new_platform,limit=1] run function mineraft:core/structure/store_item
