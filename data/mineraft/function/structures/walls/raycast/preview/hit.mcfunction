...
execute as @e[type=item_display,tag=mr.wall_preview] if score @s mr.link = #player_link mr.data store success score #type_changed mr.data run data modify entity @s data.preview_type set from storage mineraft:walls wall_type
execute if score #type_changed mr.data matches 1 as @e[type=item_display,tag=mr.wall_preview] if score @s mr.link = #player_link mr.data run kill @s
...
execute as @e[type=item_display,tag=mr.new_wall_preview,limit=1] run data modify entity @s data.preview_type set from storage mineraft:walls wall_type
